...
function SpeedrunStart()
    ...
    ClientCmd( "sc_strafe_yaw \"\";sc_strafe_type 0;-jump;pausable 0;-duck;sc_strafe_ignore_ground 1;-sc_ducktap" );
end

function OnTouchTrigger( name )
    if ( name == "spawn") then
        ClientCmd("+use;wait;-use;wait; weapon_9mmhandgun");
        ...
        CreateTimer( 7.5, ClientCmd, "fps_max 500;-attack; -sc_ducktap; sc_strafe 1; sc_strafe_yaw \"\"");
    end
...
function OnPlayerUnstuck( pPlayerEdict )
    ...
    if ( pPlayerEdict.vars.netname == "void" ) then
        ...
        printl( "void's forced unstuck at " .. tostring( pPlayerEdict.vars.origin ) );
    end
end
...
function OnBeginPlayerRevive( pPlayerEdict )
    if ( pPlayerEdict.vars.netname == "Sw1ft" ) then
        kolo_revs_count = kolo_revs_count + 1;

        if ( kolo_revs_count == 2 ) then
            -- ClientCmd( "toggle_survival_mode;toggle_survival_mode" );
            CreateTimer( 0.0, ClientCmd, "toggle_survival_mode;toggle_survival_mode" );
        end
    end
end
...
function OnSpecialSpawn( pPlayerEdict )
    ...
    pPlayerEdict.vars.origin = Vector(17.220428, -2833.981934, 1371.997437);
end
